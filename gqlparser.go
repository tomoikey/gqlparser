...
func LoadQueryWithRules(schema *ast.Schema, str string, r *rules.Rules) (*ast.QueryDocument, gqlerror.List) {
	query, err := parser.ParseQuery(&ast.Source{Input: str})
	if err != nil {
		gqlErr, ok := err.(*gqlerror.Error)
		if ok {
	...
	errs := validator.ValidateWithRules(schema, query, r)
	if len(errs) > 0 {
		return nil, errs
	}
...
func MustLoadQueryWithRules(schema *ast.Schema, str string, r *rules.Rules) *ast.QueryDocument {
	q, err := LoadQueryWithRules(schema, str, r)
	if err != nil {
		panic(err)
	}
	return q
